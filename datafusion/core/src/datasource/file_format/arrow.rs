...
mod tests {
    ...
    #[tokio::test]
    async fn test_infer_schema_from_ipc_format_stream() -> Result<()> {
        // Test both file and stream formats
    ...
    #[tokio::test]
    async fn test_infer_schema_from_short_ipc_format_stream() -> Result<()> {
        // Test both file and stream formats
        for file in ["example.arrow", "example_stream.arrow"] {
            let mut bytes = std::fs::read(format!("tests/data/{}", file))?;
